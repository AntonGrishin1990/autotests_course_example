...
# Здесь пишем код
def segment(cort1,cort2 ):
    try:
        # Проверяем, что приходит именно кортеж и содержит он ровно 2 числа
        if not (isinstance(cort1, tuple) and isinstance(cort2, tuple)):
            raise ValueError("Input points must be tuples.")
        if len(cort1) != 2 or len(cort2) != 2:
            raise ValueError("Each point must contain exactly two coordinates.")

        # Ошибок нет - возвращаем сумму координат
        return cort1[0] + cort1[1] + cort2[0] + cort2[1]

    except Exception as e:
        # Если падает исключение, возвращаем текст задом наперед
        return str(e)[::-1]
