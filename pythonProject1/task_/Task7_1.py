...
class Segment:
    ...
    def length(self):
        ...
        return round(sqrt((self.start[0] - self.finish[0]) ** 2 + (self.start[1] - self.finish[1]) ** 2), 2)

    def x_axis_intersection(self):
        """ возвращает True, если отрезок пересекает ось абцисс, иначе False"""
        if self.start[1] > 0 and self.finish[1] < 0 or self.start[1] < 0 and self.finish[1] > 0:
            return True
        else:
            return False
    ...
    def y_axis_intersection(self):
        ...
        if self.start[0] > 0 and self.finish[0] < 0 or self.start[0] < 0 and self.finish[0] > 0:
            return True
        else:
            return False
